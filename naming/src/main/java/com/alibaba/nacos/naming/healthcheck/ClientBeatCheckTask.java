...
public class ClientBeatCheckTask implements Runnable {
    ...
    public void run() {
        try {
            if (!getDistroMapper().responsible(service.getName())) { //  集群模式下，非当前机器处理的服务直接返回
                return;
            }
            ...
            List<Instance> instances = service.allIPs(true); // 获取所有客户端服务ip

            // first set health status of instances:
            for (Instance instance : instances) {
                if (System.currentTimeMillis() - instance.getLastBeat() > instance.getInstanceHeartBeatTimeOut()) { // 当前时间-上次心跳时间是否大于心跳时间间隔，默认15s
                    if (!instance.isMarked()) {
                        if (instance.isHealthy()) {
                            instance.setHealthy(false); // 实例异常
                            Loggers.EVT_LOG
            ...
            for (Instance instance : instances) {
                ...
                if (instance.isMarked()) { // 持久实例直接返回
                    continue;
                }

                if (System.currentTimeMillis() - instance.getLastBeat() > instance.getIpDeleteTimeout()) { // 删除超时时间
                    // delete instance
                    Loggers.SRV_LOG.info("[AUTO-DELETE-IP] service: {}, ip: {}", service.getName(),
                            JacksonUtils.toJson(instance));
                    deleteIp(instance); // 超过30s删除实例
                }
            }
