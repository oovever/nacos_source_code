...
public class Service extends com.alibaba.nacos.api.naming.pojo.Service implements Record, RecordListener<Instances> {
    ...
    public void processClientBeat(final RsInfo rsInfo) {
        ClientBeatProcessor clientBeatProcessor = new ClientBeatProcessor(); // 创建心跳处理器
        clientBeatProcessor.setService(this);
        clientBeatProcessor.setRsInfo(rsInfo);
        HealthCheckReactor.scheduleNow(clientBeatProcessor); // 立即执行调度
    }
    ...
    public void onChange(String key, Instances value) throws Exception {
        ...
        for (Instance instance : value.getInstanceList()) {
            ...
            if (instance.getWeight() > 10000.0D) { //权重设置
        ...
        updateIPs(value.getInstanceList(), KeyBuilder.matchEphemeralInstanceListKey(key)); // 更新本地注册表
    ...
    public void updateIPs(Collection<Instance> instances, boolean ephemeral) {
        ...
        for (String clusterName : clusterMap.keySet()) { //clusterMap clusterName与cluster的对应关系
            ipMap.put(clusterName, new ArrayList<>());
        }

        for (Instance instance : instances) { // 遍历所有实例
            try {
                if (instance == null) {
                ...
                if (StringUtils.isEmpty(instance.getClusterName())) { // 设置默认CLUSTER
                    instance.setClusterName(UtilsAndCommons.DEFAULT_CLUSTER_NAME);
                }

                if (!clusterMap.containsKey(instance.getClusterName())) { // cluster不存在创建新的cluster
                    Loggers.SRV_LOG
                ...
                List<Instance> clusterIPs = ipMap.get(instance.getClusterName()); // 根据集群名称，获取集群下的实例List
                if (clusterIPs == null) {
                    clusterIPs = new LinkedList<>();
                    ipMap.put(instance.getClusterName(), clusterIPs);
                }

                clusterIPs.add(instance); // 添加到对应集群的实例List
            } catch (Exception e) {
        ...
        for (Map.Entry<String, List<Instance>> entry : ipMap.entrySet()) {
            ...
            clusterMap.get(entry.getKey()).updateIps(entryIPs, ephemeral); // 更新集群下的实例
        }

        setLastModifiedMillis(System.currentTimeMillis()); // 设置最近修改时间s
        getPushService().serviceChanged(this); // 发布数据变更事件
     ...
     */
    public void init() {
        HealthCheckReactor.scheduleCheck(clientBeatCheckTask); // 定时调度清除过期实例的任务,心跳检测
        for (Map.Entry<String, Cluster> entry : clusterMap.entrySet()) { // 当前服务下所有集群的健康检查任务
            entry.getValue().setService(this);
            entry.getValue().init(); // 为集群节点创建健康检查服务
        }
    }
    ...
    public synchronized void recalculateChecksum() {
        ...
        checksum = MD5Utils.md5Hex(ipsString.toString(), Constants.ENCODE); // 实例拼接后计算ip md5
    }
