...
public class DistroDataStorageImpl implements DistroDataStorage {
    ...
    public DistroData getDatumSnapshot() {
        Map<String, Datum> result = dataStore.getDataMap(); // 缓存中获取所有数据实例
        byte[] dataContent = ApplicationUtils.getBean(Serializer.class).serialize(result); // 序列化后存储
    ...
    public DistroData getVerifyData() {
        ...
        for (String key : dataStore.keys()) {
            if (!distroMapper.responsible(KeyBuilder.getServiceName(key))) { // 不是本机处理返回
                continue;
            }
            ...
            keyChecksums.put(key, datum.value.getChecksum()); // 记录key下所有实例的checkSum
        }
        if (keyChecksums.isEmpty()) {
            return null;
        }
        DistroKey distroKey = new DistroKey("checksum", KeyBuilder.INSTANCE_LIST_KEY_PREFIX);
        return new DistroData(distroKey, ApplicationUtils.getBean(Serializer.class).serialize(keyChecksums)); // 构建DistroData对象，存储checksum
    }
}
