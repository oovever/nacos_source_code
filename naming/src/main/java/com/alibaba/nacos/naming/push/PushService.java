...
public class PushService implements ApplicationContextAware, ApplicationListener<ServiceChangeEvent> {
    ...
    static {
        try {
            udpSocket = new DatagramSocket(); // 创建UDP客户端

            Receiver receiver = new Receiver(); // 创建receiver任务

            Thread inThread = new Thread(receiver);
            inThread.setDaemon(true);
            inThread.setName("com.alibaba.nacos.naming.push.receiver");
            inThread.start(); // 接受客户端ack请求
//        20执行一次
            GlobalExecutor.scheduleRetransmitter(() -> {
                try {
                    removeClientIfZombie(); // 移除僵尸客户端
                } catch (Throwable e) {
    ...
    @Override
    public void onApplicationEvent(ServiceChangeEvent event) { // 发布变更事件后触发此方法执行
...
//       启动定时任务，异步执行任务
        Future future = GlobalExecutor.scheduleUdpSender(() -> {
            try {
                ...
                ConcurrentMap<String, PushClient> clients = clientMap
                        .get(UtilsAndCommons.assembleFullServiceName(namespaceId, serviceName)); // 获取微服务对应的所有clients map
                ...
                for (PushClient client : clients.values()) {
                    if (client.zombie()) { // 当前客户端是否已经超时，僵尸客户端
                        Loggers.PUSH.debug("client is zombie: " + client.toString());
                        clients.remove(client.toString()); // 移除当前客户端
                        Loggers.PUSH.debug("client is zombie: " + client.toString());
                        continue;
                    }
                    ...
                    String key = getPushCacheKey(serviceName, client.getIp(), client.getAgent()); // 服务 IP 客户端唯一确定key
                    byte[] compressData = null;
                    Map<String, Object> data = null;
                    if (switchDomain.getDefaultPushCacheMillis() >= 20000 && cache.containsKey(key)) { // 发送缓存时间大于20s，并且缓存中包含该key
    ...
    public void addClient(String namespaceId, String serviceName, String clusters, String agent,
        ...
        PushClient client = new PushClient(namespaceId, serviceName, clusters, agent, socketAddr, dataSource, tenant,
                app); // 创建UDP PUSH client
        addClient(client);  // 添加client到clientMap中  ConcurrentMap<String, ConcurrentMap<String, PushClient>>
    }
    ...
    public void addClient(PushClient client) {
        ...
        String serviceKey = UtilsAndCommons.assembleFullServiceName(client.getNamespaceId(), client.getServiceName()); // 生成service key namespaceId##微服务名称
//       clientMap存放所有实例对应的udp client，key namespaceId##groupId@@微服务名称
//        value 第二层map key  sb.append("serviceName: ").append(serviceName).append(", clusters: ").append(clusters).append(", address: ")
//                    .append(socketAddr).append(", agent: ").append(agent);，value PushClient
        ConcurrentMap<String, PushClient> clients = clientMap.get(serviceKey);
        if (clients == null) { // 如果第二层map为空，则初始化
            clientMap.putIfAbsent(serviceKey, new ConcurrentHashMap<>(1024));
            clients = clientMap.get(serviceKey);
        }

        PushClient oldClient = clients.get(client.toString());
        if (oldClient != null) { // 之前存在client
            oldClient.refresh(); // 更新 lastRefTime
        } else {  // 之前不存在
            PushClient res = clients.putIfAbsent(client.toString(), client); // 写入新client
            if (res != null) {
    ...
    private static void removeClientIfZombie() {
        ...
        for (Map.Entry<String, ConcurrentMap<String, PushClient>> entry : clientMap.entrySet()) {
            ...
            for (Map.Entry<String, PushClient> entry1 : clientConcurrentMap.entrySet()) {
                ...
                if (client.zombie()) { // 超时
                    clientConcurrentMap.remove(entry1.getKey()); // 移除
                }
            }
    ...
    private static Receiver.AckEntry prepareAckEntry(PushClient client, Map<String, Object> data, long lastRefTime) {
        ...
        try {
            ...
            dataBytes = compressIfNecessary(dataBytes); // 有必要则进行压缩
    ...
    public static String getPushCacheKey(String serviceName, String clientIP, String agent) {
        return serviceName + UtilsAndCommons.CACHE_KEY_SPLITER + agent; //没有用到IP
    }
    ...
    public void serviceChanged(Service service) {
        ...
        if (futureMap
                .containsKey(UtilsAndCommons.assembleFullServiceName(service.getNamespaceId(), service.getName()))) { // 合并变更事件，减少推送频率 futureMap是否包含nameSpaceId##服务名称
            return;
        }

        this.applicationContext.publishEvent(new ServiceChangeEvent(this, service)); // 发布服务变更事件
    }
    ...
    private static byte[] compressIfNecessary(byte[] dataBytes) throws IOException {
        ...
        if (dataBytes.length < maxDataSizeUncompress) { // 大小小于1KB不压缩
            return dataBytes;
        }

        ByteArrayOutputStream out = new ByteArrayOutputStream();
        GZIPOutputStream gzip = new GZIPOutputStream(out);
        gzip.write(dataBytes); // gzip压缩
        gzip.close();
    ...
    private static Receiver.AckEntry udpPush(Receiver.AckEntry ackEntry) { // 发送UDP通知
        if (ackEntry == null) {
            Loggers.PUSH.error("[NACOS-PUSH] ackEntry is null.");
            return null;
        }

        if (ackEntry.getRetryTimes() > MAX_RETRY_TIMES) { // 重试次数超过最大值
            Loggers.PUSH.warn("max re-push times reached, retry times {}, key: {}", ackEntry.retryTimes, ackEntry.key);
            ackMap.remove(ackEntry.key); // ackMap移除UDP连接
            udpSendTimeMap.remove(ackEntry.key); // udpSendTimeMap移除UDP连接
            failedPush += 1; // 失败次数加1
            return ackEntry;
        }

        try {
            if (!ackMap.containsKey(ackEntry.key)) { // ackMap不包含这个key，发送计数+1
                totalPush++;
            }
            ackMap.put(ackEntry.key, ackEntry); // 记录已发送的
            udpSendTimeMap.put(ackEntry.key, System.currentTimeMillis());

            Loggers.PUSH.info("send udp packet: " + ackEntry.key);
            udpSocket.send(ackEntry.origin); // 发送udp请求

            ackEntry.increaseRetryTime();

            GlobalExecutor.scheduleRetransmitter(new Retransmitter(ackEntry),
                    TimeUnit.NANOSECONDS.toMillis(ACK_TIMEOUT_NANOS), TimeUnit.MILLISECONDS); // 定时任务，推送失败后重新推送
    ...
    public static class Receiver implements Runnable {
        ...
        public void run() {
            while (true) {
                ...
                try {
                    udpSocket.receive(packet); // 接收客户端响应请求

                    String json = new String(packet.getData(), 0, packet.getLength(), StandardCharsets.UTF_8).trim(); // 序列化解析
                    AckPacket ackPacket = JacksonUtils.toObj(json, AckPacket.class);

                    InetSocketAddress socketAddress = (InetSocketAddress) packet.getSocketAddress();
                    String ip = socketAddress.getAddress().getHostAddress(); // 获取IP和端口
                    int port = socketAddress.getPort();

                    if (System.nanoTime() - ackPacket.lastRefTime > ACK_TIMEOUT_NANOS) { // 请求耗时超过10s
                    ...
                    AckEntry ackEntry = ackMap.remove(ackKey); // 移除收到响应的key
                    if (ackEntry == null) {
                    ...
                    pushCostMap.put(ackKey, pushCost); // 记录推送耗时

                    udpSendTimeMap.remove(ackKey); // 移除推送事件
