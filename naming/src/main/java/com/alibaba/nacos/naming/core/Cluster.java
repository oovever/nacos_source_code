...
public class Cluster extends com.alibaba.nacos.api.naming.pojo.Cluster implements Cloneable {
    ...
    @JsonIgnore
    private Set<Instance> persistentInstances = new HashSet<>(); // 持久实例Instance集合

    @JsonIgnore
    private Set<Instance> ephemeralInstances = new HashSet<>();  // 临时实例Instance集合
    ...
    public void init() {
        ...
        checkTask = new HealthCheckTask(this); // 创建健康检查服务

        HealthCheckReactor.scheduleCheck(checkTask); // 异步执行任务
        inited = true; // 记录集群已经初始化
    }
    ...
    public void updateIps(List<Instance> ips, boolean ephemeral) {
        ...
        Set<Instance> toUpdateInstances = ephemeral ? ephemeralInstances : persistentInstances;  //cow机制先复制一份

        HashMap<String, Instance> oldIpMap = new HashMap<>(toUpdateInstances.size());

        for (Instance ip : toUpdateInstances) {
            oldIpMap.put(ip.getDatumKey(), ip); // old map中存储
        }
        ...
        if (ephemeral) {
            ephemeralInstances = toUpdateInstances; // 注册实例存储
        } else {
    ...
    private List<Instance> updatedIps(Collection<Instance> newInstance, Collection<Instance> oldInstance) {
        ...
        List<Instance> intersects = (List<Instance>) CollectionUtils.intersection(newInstance, oldInstance); // 获取实例的交集
        ...
        for (Instance instance : oldInstance) {
            if (stringIpAddressMap.containsKey(instance.getIp() + ":" + instance.getPort())) { // 交集中包含当前实例
