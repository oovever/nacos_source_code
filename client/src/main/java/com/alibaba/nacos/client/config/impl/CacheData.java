...
public class CacheData {
    ...
    public void addListener(Listener listener) {
        ...
        ManagerListenerWrap wrap =
                (listener instanceof AbstractConfigChangeListener) ? new ManagerListenerWrap(listener, md5, content)
                        : new ManagerListenerWrap(listener, md5); // 包装

        if (listeners.addIfAbsent(wrap)) { // warp添加到cacheData的监听器集合中
    ...
    void checkListenerMd5() {
        for (ManagerListenerWrap wrap : listeners) {
            if (!md5.equals(wrap.lastCallMd5)) { // 遍历计算md5
                safeNotifyListener(dataId, group, content, type, md5, wrap); // 发送变更通知
            }
        }
    }
    ...
    private void safeNotifyListener(final String dataId, final String group, final String content, final String type,
        ...
        Runnable job = new Runnable() {
            @Override
            public void run() { // 创建job对象，异步执行
                ClassLoader myClassLoader = Thread.currentThread().getContextClassLoader();
                ClassLoader appClassLoader = listener.getClass().getClassLoader();
                try {
                    if (listener instanceof AbstractSharedListener) { // AbstractSharedListener处理
        ...
        try {
            if (null != listener.getExecutor()) {
                listener.getExecutor().execute(job); // 执行任务
            } else {
                job.run();
            }
        } catch (Throwable t) {
