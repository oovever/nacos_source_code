...
public class LocalConfigInfoProcessor {
    ...
    public static String getFailover(String serverName, String dataId, String group, String tenant) { // 获取本地配置
        File localPath = getFailoverFile(serverName, dataId, group, tenant); // 本地文件路径
        if (!localPath.exists() || !localPath.isFile()) {
            return null;
        }

        try {
            return readFile(localPath); // 读取本地配置
