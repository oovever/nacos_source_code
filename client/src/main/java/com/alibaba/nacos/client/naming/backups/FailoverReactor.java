...
public class FailoverReactor implements Closeable {
    ...
    public FailoverReactor(HostReactor hostReactor, String cacheDir) {
        ...
        this.failoverDir = cacheDir + "/failover"; // 故障转移文件目录 ${user.home}/nacos/naming/public/failover
    ...
    public void init() {
        ...
        executorService.scheduleWithFixedDelay(new SwitchRefresher(), 0L, 5000L, TimeUnit.MILLISECONDS); // 初始化立即执行，5s调度一次
        ...
        executorService.schedule(new Runnable() {
            ...
            public void run() {
                try {
                    ...
                    if (files == null || files.length <= 0) { // 文件为空或者文件长度小于0，执行DiskFileWriter任务
        ...
        }, 10000L, TimeUnit.MILLISECONDS); // 10秒后如果故障目录为空，则执行DiskFileWriter任务强制备份
    }
    ...
    class SwitchRefresher implements Runnable {
        ...
        public void run() {
            try {
                ...
                if (!switchFile.exists()) { // 故障转移文件不存在直接返回
                ...
                if (lastModifiedMillis < modified) { // 文件已经修改
                    lastModifiedMillis = modified;
                    String failover = ConcurrentDiskUtil.getFileContent(failoverDir + UtilAndComs.FAILOVER_SWITCH,
                            Charset.defaultCharset().toString()); // 获取文件内容
                    if (!StringUtils.isEmpty(failover)) {
                        String[] lines = failover.split(DiskCache.getLineSeparator());

                        for (String line : lines) {
                            String line1 = line.trim();
                            if ("1".equals(line1)) { // 开启故障转移
                                switchParams.put("failover-mode", "true");
                                NAMING_LOGGER.info("failover-mode is on");
                                new FailoverFileReader().run(); // 执行FailoverFileReader线程
                            } else if ("0".equals(line1)) { // 关闭故障转移
                                switchParams.put("failover-mode", "false");
    ...
    class FailoverFileReader implements Runnable {
        ...
        public void run() {
            ...
            try {
                ...
                File[] files = cacheDir.listFiles(); // failover目录下的所有文件
                if (files == null) { // 文件不存在跳过
                    return;
                }
                ...
                for (File file : files) {
                    ...
                    if (file.getName().equals(UtilAndComs.FAILOVER_SWITCH)) { // 故障转移开关标志文件，跳过
                        continue;
                    }
                    ...
                    try {
                        String dataString = ConcurrentDiskUtil
                                .getFileContent(file, Charset.defaultCharset().toString()); // 读取文件内容
    ...
    class DiskFileWriter extends TimerTask {
        ...
        public void run() {
            ...
            for (Map.Entry<String, ServiceInfo> entry : map.entrySet()) {
                ...
                DiskCache.write(serviceInfo, failoverDir); // 将ServiceInfo写入备份磁盘
            }
        }
    }
