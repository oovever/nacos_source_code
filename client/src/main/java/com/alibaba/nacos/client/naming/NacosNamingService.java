...
public class NacosNamingService implements NamingService {
    ...
    private void init(Properties properties) throws NacosException {
        ...
        this.serverProxy = new NamingProxy(this.namespace, this.endpoint, this.serverList, properties); // 初始化proxy代理
        this.beatReactor = new BeatReactor(this.serverProxy, initClientBeatThreadCount(properties)); // 心跳
    ...
    public void registerInstance(String serviceName, String ip, int port) throws NacosException {
        registerInstance(serviceName, ip, port, Constants.DEFAULT_CLUSTER_NAME); // 客户端注册实例
    }
    ...
    public void registerInstance(String serviceName, String groupName, Instance instance) throws NacosException {
        ...
        String groupedServiceName = NamingUtils.getGroupedName(serviceName, groupName); // 获取微服务名称groupId@微服务名称
        if (instance.isEphemeral()) { // 是否临时实例
            BeatInfo beatInfo = beatReactor.buildBeatInfo(groupedServiceName, instance); // 构建请求心跳实例
            beatReactor.addBeatInfo(groupedServiceName, beatInfo);// 向server发送心跳信息
        }
        serverProxy.registerService(groupedServiceName, groupName, instance); // server注册
    }
    ...
    public List<Instance> getAllInstances(String serviceName) throws NacosException {
        return getAllInstances(serviceName, new ArrayList<String>()); // 获取服务实例
    }
    ...
    @Override
    public List<Instance> getAllInstances(String serviceName, String groupName, List<String> clusters,
            boolean subscribe) throws NacosException { // 获取所有实例
        ...
        if (subscribe) { // 订阅处理
            ...
            serviceInfo = hostReactor
                    .getServiceInfoDirectlyFromServer(NamingUtils.getGroupedName(serviceName, groupName),
                            StringUtils.join(clusters, ",")); // 通过server获取serverInfo信息
        }
        List<Instance> list;
    ...
    public List<Instance> selectInstances(String serviceName, String groupName, List<String> clusters, boolean healthy,
        ...
        if (subscribe) {
            serviceInfo = hostReactor.getServiceInfo(NamingUtils.getGroupedName(serviceName, groupName),
                    StringUtils.join(clusters, ",")); // 获取实例
        } else {
            serviceInfo = hostReactor
                    .getServiceInfoDirectlyFromServer(NamingUtils.getGroupedName(serviceName, groupName),
                            StringUtils.join(clusters, ","));
    ...
    private List<Instance> selectInstances(ServiceInfo serviceInfo, boolean healthy) {
        ...
        Iterator<Instance> iterator = list.iterator(); // 获取服务所有实例
        while (iterator.hasNext()) {
            Instance instance = iterator.next();
            if (healthy != instance.isHealthy() || !instance.isEnabled() || instance.getWeight() <= 0) {
                iterator.remove(); // 删除不健康实例
            }
        }
    ...
    public ListView<String> getServicesOfServer(int pageNo, int pageSize, String groupName) throws NacosException {
        return getServicesOfServer(pageNo, pageSize, groupName, null); // 获取groupName的服务
    }
    ...
    public ListView<String> getServicesOfServer(int pageNo, int pageSize, String groupName, AbstractSelector selector)
            throws NacosException {
        return serverProxy.getServiceList(pageNo, pageSize, groupName, selector); // 获取所有服务列表
    }
