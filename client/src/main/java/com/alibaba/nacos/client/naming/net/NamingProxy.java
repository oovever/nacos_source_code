...
public class NamingProxy implements Closeable {
    ...
    public NamingProxy(String namespaceId, String endpoint, String serverList, Properties properties) {
        ...
        if (StringUtils.isNotEmpty(serverList)) {
            ...
            if (this.serverList.size() == 1) {
                this.nacosDomain = serverList; // 只有一个服务nacosDomain设置为服务
            }
        }
    ...
    public void registerService(String serviceName, String groupName, Instance instance) throws NacosException {
        ...
        params.put(CommonParams.NAMESPACE_ID, namespaceId); // 初始化请求参数
        ...
        reqApi(UtilAndComs.nacosUrlInstance, params, HttpMethod.POST); // 发送注册请求
    ...
    public String queryList(String serviceName, String clusters, int udpPort, boolean healthyOnly)
        ...
        params.put("udpPort", String.valueOf(udpPort)); //udp端口,订阅模式下接受通知所用的端口
        params.put("clientIP", NetUtils.localIP());
        params.put("healthyOnly", String.valueOf(healthyOnly));

        return reqApi(UtilAndComs.nacosUrlBase + "/instance/list", params, HttpMethod.GET); // 调用/nacos/v1/ns/instance/list方法
    }
    ...
    public JsonNode sendBeat(BeatInfo beatInfo, boolean lightBeatEnabled) throws NacosException {
        ...
        String result = reqApi(UtilAndComs.nacosUrlBase + "/instance/beat", params, bodyMap, HttpMethod.PUT); // 发送心跳请求
    ...
    public ListView<String> getServiceList(int pageNo, int pageSize, String groupName, AbstractSelector selector)
        ...
        params.put(CommonParams.NAMESPACE_ID, namespaceId);
        params.put(CommonParams.GROUP_NAME, groupName); // 拼接参数
        ...
        String result = reqApi(UtilAndComs.nacosUrlBase + "/service/list", params, HttpMethod.GET); // get请求获取服务列表
    ...
    public String reqApi(String api, Map<String, String> params, Map<String, String> body, String method)
            throws NacosException {
        return reqApi(api, params, body, getServerList(), method); // getServerList获取服务列表
    }
    ...
    public String reqApi(String api, Map<String, String> params, Map<String, String> body, List<String> servers,
        ...
        params.put(CommonParams.NAMESPACE_ID, getNamespaceId()); // 添加命名空间
        ...
        if (StringUtils.isNotBlank(nacosDomain)) { // 只有一台机器nacosDomain不为空，集群时nacosDomain为空
            for (int i = 0; i < maxRetry; i++) { // 重试连接
                try {
                    return callServer(api, params, body, nacosDomain, method); // 调用服务
            ...
            int index = random.nextInt(servers.size()); // 获取随机索引

            for (int i = 0; i < servers.size(); i++) {
                String server = servers.get(index); // 从集群中随机选一台机器
                try {
                    return callServer(api, params, body, server, method); // 调用远程服务
                } catch (NacosException e) {
                    exception = e;
                    if (NAMING_LOGGER.isDebugEnabled()) {
                ...
                index = (index + 1) % servers.size(); // 调用失败，重新选一台机器
            }
        }
    ...
    public String callServer(String api, Map<String, String> params, Map<String, String> body, String curServer,
        ...
        if (curServer.startsWith(UtilAndComs.HTTPS) || curServer.startsWith(UtilAndComs.HTTP)) {
            url = curServer + api; //有http协议直接连接
        } else {
            if (!IPUtil.containsPort(curServer)) {
                curServer = curServer + IPUtil.IP_PORT_SPLITER + serverPort; // 拼接IP和端口
            }
        ...
        try {
            HttpRestResult<String> restResult = nacosRestTemplate
                    .exchangeForm(url, header, Query.newInstance().initParams(params), body, method, String.class);// 调用nacosRestTemplate发送请求,HttpClientRequest 对JDK中httpClient的封装
