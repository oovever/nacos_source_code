...
public class NacosConfigService implements ConfigService {
    ...
    public NacosConfigService(Properties properties) throws NacosException {
        ValidatorUtils.checkInitParam(properties); // 校验属性参数
        ...
        this.agent = new MetricsHttpAgent(new ServerHttpAgent(properties)); // httpclient
        this.agent.start();
        this.worker = new ClientWorker(this.agent, this.configFilterChainManager, properties); // 创建worker
    }
    ...
    public String getConfig(String dataId, String group, long timeoutMs) throws NacosException {
        return getConfigInner(namespace, dataId, group, timeoutMs); // 加载配置
    }
...
//        获取属性信息
        ...
        // 优先使用本地配置， 配置中心挂了，本地有备份;本地配置文件默认是不存在的，如果需要使用本地配置文件覆盖远程配置文件则需要自己新建本地配置文件
        ...
        if (content != null) {
            ...
            return content; // 本地存在直接返回
        }

        try {
            String[] ct = worker.getServerConfig(dataId, group, tenant, timeoutMs); // 远程拉取配置 worker配置中心工作线程
            cr.setContent(ct[0]); // 设置配置

            configFilterChainManager.doFilter(null, cr);
            content = cr.getContent();

            return content; // 有直接返回
        ...
        LOGGER.warn("[{}] [get-config] get snapshot ok, dataId={}, group={}, tenant={}, config={}", agent.getName(),
                dataId, group, tenant, ContentUtils.truncateContent(content));
        content = LocalConfigInfoProcessor.getSnapshot(agent.getName(), dataId, group, tenant); // 获取快照配置
        cr.setContent(content);
        configFilterChainManager.doFilter(null, cr);
        content = cr.getContent();
        return content;
