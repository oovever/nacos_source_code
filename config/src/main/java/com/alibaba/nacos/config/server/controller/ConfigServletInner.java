...
public class ConfigServletInner {
    ...
    /**
     * @clientMd5Map test+DEFAULT_GROUP -> 9a0364b9e99bb480dd25e1f0284c8555
     * 轮询接口.
     */
    public String doPollingConfig(HttpServletRequest request, HttpServletResponse response,
        ...
        // Long polling.
        if (LongPollingService.isSupportLongPolling(request)) { // 是否是长轮询处理，请求头包含长轮询timeOut
            longPollingService.addLongPollingClient(request, response, clientMd5Map, probeRequestSize); // 长轮询处理
            return HttpServletResponse.SC_OK + "";
        }
//      短轮询处理
        // Compatible with short polling logic.
        List<String> changedGroups = MD5Util.compareMd5(request, response, clientMd5Map); // 获取目标配置中发生变更的文件key
        ...
        // Befor 2.0.4 version, return value is put into header. 不同版本返回不同的响应形式
    ...
    public String doGetConfig(HttpServletRequest request, HttpServletResponse response, String dataId, String group,
        ...
        int lockResult = tryConfigReadLock(groupKey); // 获取锁，获取不到自选重试10次
        ...
        if (lockResult > 0) {
            ...
            try {
                ...
                if (isBeta) {
                    ...
                    if (PropertyUtil.isDirectRead()) { // 从mysql中获取
                        configInfoBase = persistService.findConfigInfo4Beta(dataId, group, tenant);
                    } else {
                        file = DiskUtil.targetBetaFile(dataId, group, tenant); // 从文件中获取
                    }
