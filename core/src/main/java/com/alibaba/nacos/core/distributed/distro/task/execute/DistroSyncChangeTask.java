...
public class DistroSyncChangeTask extends AbstractDistroExecuteTask {
    ...
    public void run() {
        ...
        try {
            String type = getDistroKey().getResourceType(); // 获取资源类型
            DistroData distroData = distroComponentHolder.findDataStorage(type).getDistroData(getDistroKey()); //Nacos启动时缓存在dataStorageMap中两种类型处理器分别用于处理gRPC和HTTP通信方式， 从DistroClientDataProcessor获取DistroData
