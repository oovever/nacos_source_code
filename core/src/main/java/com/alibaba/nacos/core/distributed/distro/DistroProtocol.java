...
public class DistroProtocol {
    ...
    private void startDistroTask() {
        ...
        startVerifyTask(); // 用于集群启动或节点加入新集群同步数据 定时校验
        startLoadTask(); // 用于在运行时，各节点之间确保数据一致性的校验 刚启动时同步
    }
    ...
    private void startVerifyTask() {
        GlobalExecutor.schedulePartitionDataTimedSync(new DistroVerifyTask(memberManager, distroComponentHolder),
                distroConfig.getVerifyIntervalMillis()); // 5s校验本地数据与其余nacos服务数据的一致性
    }
    ...
    public void sync(DistroKey distroKey, DataOperation action, long delay) {
        for (Member each : memberManager.allMembersWithoutSelf()) { // 除自己的机器同步
            DistroKey distroKeyWithTarget = new DistroKey(distroKey.getResourceKey(), distroKey.getResourceType(),
                    each.getAddress()); // 构造DistroKey
            DistroDelayTask distroDelayTask = new DistroDelayTask(distroKeyWithTarget, action, delay); //封装延时任务
            distroTaskEngineHolder.getDelayTaskExecuteEngine().addTask(distroKeyWithTarget, distroDelayTask); // 添加数据到tasks，目标机器、任务
    ...
    public DistroData queryFromRemote(DistroKey distroKey) {
        ...
        return transportAgent.getData(distroKey, distroKey.getTargetServer()); // 远程获取数据
    }
    ...
    public boolean onReceive(DistroData distroData) {
        String resourceType = distroData.getDistroKey().getResourceType(); // 获取资源类型
    ...
    public boolean onVerify(DistroData distroData) {
        ...
        DistroDataProcessor dataProcessor = distroComponentHolder.findDataProcessor(resourceType); // 不同数据类型做不同处理
        if (null == dataProcessor) {
            Loggers.DISTRO.warn("[DISTRO] Can't find verify data process for received data {}", resourceType);
            return false;
        }
        return dataProcessor.processVerifyData(distroData); // 处理校验数据
    }
    ...
    public DistroData onSnapshot(String type) {
        DistroDataStorage distroDataStorage = distroComponentHolder.findDataStorage(type); // 根据存储类型获取distroDataStorage
        if (null == distroDataStorage) {
        ...
        return distroDataStorage.getDatumSnapshot(); // 获取这个存储类型下的所有数据
    }
}
