...
public class DistroDelayTask extends AbstractDelayTask {
    ...
    @Override
    public void merge(AbstractDelayTask task) { // NacosDelayTaskExecuteEngine#addTask不保证添加顺序
        if (!(task instanceof DistroDelayTask)) {
            return;
        }
        DistroDelayTask newTask = (DistroDelayTask) task;
        if (!action.equals(newTask.getAction()) && createTime < newTask.getCreateTime()) { // 新任务已经过时，使用旧任务的操作和创建时间
