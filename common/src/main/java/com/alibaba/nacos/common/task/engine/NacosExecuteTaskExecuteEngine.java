...
public class NacosExecuteTaskExecuteEngine extends AbstractNacosTaskExecuteEngine<AbstractExecuteTask> {
    ...
    public void addTask(Object tag, AbstractExecuteTask task) {
        NacosTaskProcessor processor = getProcessor(tag); // 获取processor
        if (null != processor) {
            processor.process(task); // 执行处理任务
            return;
        }
        TaskExecuteWorker worker = getWorker(tag); // 获取worker hash取模
        worker.process(task); // worker执行任务
    }
